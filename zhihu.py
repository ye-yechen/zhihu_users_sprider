...
class ZhihuSprider(object):
    ...
    headers = {
        ...
        "Host": "www.zhihu.com",
        "authorization": "Bearer Mi4wQUZBQWtBM0MyZ2tBTUlLVGx6bWxDeGNBQUFCaEFsVk5nUXNqV1FDelBKX3dlS0d0MkcxSERIRERCb3lBeVZrRllR|1492876932|c8179588b8c59c82b9ec66488b9cabf4abb37f58",
    ...
    def __init__(self):
        ...
        self.start_url_token = 'miloyip'  # 起始用户的 url_token
        # 用户关注列表的url
...
cnt = 1
sprider = ZhihuSprider()
response = sprider.login('', '')
if response.status_code == 200:
    start_time = datetime.datetime.now()
    print '知乎登录成功, 现在开始抓取信息...'.decode('utf-8').encode('gbk')
    # 将起始用户的 url_token 加入 redis
    sprider.redis_con.lpush('redis_db_user', sprider.start_url_token)
    sprider.bfs_search()
    end_time = datetime.datetime.now()
    print '抓取完成, 共耗时 %s '.decode('utf-8').encode('gbk') % str(end_time-start_time)
